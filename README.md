# 💫 About Me:

**Android Developer & Aspiring Software Engineer** specializing in creating native, high-performance mobile experiences, robust Full-Stack Web architectures, and intelligent Machine Learning solutions.

* 🎥 **Cinema Buff:** Ask me about cinema and I'll start yapping.
* 🛠️ **Tech Enthusiast:** I love dissecting the "behind the scenes" of complex systems.
* 💬 **Current Focus:** Building production-grade mobile apps across Android (Kotlin), Flutter, and React Native.

## 🌐 Socials:

[![Instagram](https://img.shields.io/badge/Instagram-%23E4405F.svg?logo=Instagram&logoColor=white)](https://instagram.com/imkritvm) [![LinkedIn](https://img.shields.io/badge/LinkedIn-%23007BB5.svg?logo=linkedin&logoColor=white)](https://linkedin.com/in/kirti-vardhan-mishra) [![X](https://img.shields.io/badge/X-black.svg?logo=X&logoColor=white)](https://x.com/imkritvm) [![email](https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white)](mailto:mishra.kritvardhan@gmail.com)

---

## 🎯 What I'm Building Towards

I am working towards becoming a **multi-platform mobile developer** — capable of shipping production apps across all three major ecosystems:

| Platform | Framework | Status |
|----------|-----------|--------|
| 🤖 Android Native | Kotlin + Jetpack | Active — 2 published apps |
| 🦋 Cross-Platform | Flutter + Dart | Active — 1 live project |
| ⚛️ Cross-Platform | React Native + TypeScript | Learning — building first project |

---

## 💪 Tech Strength

> Scores reflect project depth, code quality, and real-world usage — not just familiarity.

```
🟣 Kotlin / Android     ████████████████████░░░░  85%   MVVM · Room · Jetpack · Chaquopy ML
🔵 React / Next.js      ██████████████████░░░░░░  78%   SSG · TypeScript · Auth · Vercel
🟢 Node / Express       ████████████████░░░░░░░░  72%   REST · MongoDB · Socket.io · JWT
🟡 Python / AI / ML     ███████████████░░░░░░░░░  65%   FastAPI · RAG · Chaquopy · Pandas
🟠 Java / Spring Boot   █████████████░░░░░░░░░░░  55%   REST API · JPA · Patient Systems
🔵 Flutter / Dart       ███████████░░░░░░░░░░░░░  45%   Riverpod · FastAPI Backend · RAG
🔴 React Native         ██░░░░░░░░░░░░░░░░░░░░░░  15%   In progress — first project building
```

---

## 🚀 Projects

### 📱 Mobile Development

#### [SyncTax](https://github.com/krit-vardhan-mishra/SyncTax) | *ML-Based Music Player*

* Architected an **offline-first** Android player with privacy-preserving, on-device ML.
* Used **Chaquopy** to run Python-based recommendation models directly on Kotlin agents.
* **Tech:** `Kotlin` `MVVM` `Room DB` `Chaquopy` `MediaPlayer API`

#### [File-Flip](https://github.com/krit-vardhan-mishra/File-Flip) | *Android & Productivity*

* An offline Android editor and converter. Edit Markdown, JSON, CSV and more.
* Features in-app previews and styled PDF exports for ultimate productivity.
* **Tech:** `Kotlin` `Jetpack` `iText PDF` `File Provider API`

#### [Sarathi-Ai](https://github.com/krit-vardhan-mishra/sarathy.ai) | *Flutter & FastAPI Backend*

* Developed an AI-driven career guidance platform leveraging RAG and Bhagavad Gita wisdom.
* Integrated a robust Python backend with a recommendation engine for precise career paths.
* **Tech:** `Flutter` `Dart` `FastAPI` `RAG` `Riverpod`

### 🌐 Web Development

#### [Blog Web App](https://github.com/krit-vardhan-mishra/blog-web-app) | *Full-Stack Web* | [Live Demo](https://blog-web-app-krit.vercel.app/)

* Full-stack blogging platform with authentication, error boundaries, and modern React architecture.
* Features user authentication and content management.
* **Tech:** `MERN Stack` `JWT Auth` `React` `Express` `MongoDB`

#### [Bank-Kiosk](https://github.com/krit-vardhan-mishra/bank-kiosk) | *AI & Multimodal Integration*

* A comprehensive banking AI kiosk and agent dashboard for seamless customer interactions.
* Integrates RAG, multimodal (voice and text), and multilingual support for Indian languages.
* **Tech:** `Python` `RAG` `Speech Recognition` `Hindi NLP`

#### [Campus-Whisper](https://github.com/krit-vardhan-mishra/Campus-Whisper) | *Real-time Communications* | [Live Demo](https://campus-whisper-kohl.vercel.app/)

* Real-time anonymous campus chat application with dynamic UI.
* **Tech:** `React` `Socket.io` `Node.js` `MongoDB`

#### [Real-Time Chat App](https://github.com/krit-vardhan-mishra/real-time-chat-app) | *Featured* | [Live Demo](https://krit-real-time-chat-app.up.railway.app/)

* Scalable messaging platform built with **MERN Stack** and **Socket.io** for instant, bi-directional communication.
* **Tech:** `TypeScript` `MERN` `Socket.io` `WebRTC` `E2E Encryption`

#### [Incisive](https://github.com/krit-vardhan-mishra/incisive-cul-private) | *Full-Stack Web* | [Live Demo](https://incisive-cul-private.vercel.app/)

* Redesigned the "Center for Unlearning and Learning" site using **Next.js 15** and TypeScript.
* Optimized performance via **SSG (Static Site Generation)** and Tailwind CSS.
* **Tech:** `Next.js 15` `TypeScript` `Tailwind CSS` `SSG` `Vercel`

#### [Well-being Waitlist](https://github.com/krit-vardhan-mishra/Well-being-Waitlist) | *Hospital Management*

* Built a patient prioritization system using **Spring-Boot** and React.
* Automated emergency level assignment using a Python AI model.
* **Tech:** `Spring Boot` `React` `PostgreSQL` `Python AI Model`

### 💻 DSA Practice & Competitive Programming

#### [Kunal Kushwaha DSA Bootcamp](https://github.com/krit-vardhan-mishra/Kunal-Kushwaha-DSA-Bootcamp-Assignment)

* Comprehensive Data Structures & Algorithms solutions following Kunal Kushwaha's bootcamp curriculum.

#### [Strivers A2Z DSA Sheet](https://github.com/krit-vardhan-mishra/Strivers-A2Z-DSA-Sheet)

* Complete solutions for Strivers A2Z DSA Sheet covering graphs, trees, dynamic programming, and more.

---

## 💻 Tech Stack:

**Languages:**
![Java](https://img.shields.io/badge/java-%23ED8B00.svg?style=flat&logo=openjdk&logoColor=white)
![Kotlin](https://img.shields.io/badge/kotlin-%237F52FF.svg?style=flat&logo=kotlin&logoColor=white)
![Python](https://img.shields.io/badge/python-3670A0?style=flat&logo=python&logoColor=ffdd54)
![TypeScript](https://img.shields.io/badge/typescript-%23007ACC.svg?style=flat&logo=typescript&logoColor=white)
![C++](https://img.shields.io/badge/c++-%2300599C.svg?style=flat&logo=c%2B%2B&logoColor=white)
![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=flat&logo=javascript&logoColor=%23F7DF1E)
![Dart](https://img.shields.io/badge/dart-%230175C2.svg?style=flat&logo=dart&logoColor=white)

**Frameworks & Libraries:**
![Next JS](https://img.shields.io/badge/Next-black?style=flat&logo=next.js&logoColor=white)
![React](https://img.shields.io/badge/react-%2320232a.svg?style=flat&logo=react&logoColor=%2361DAFB)
![Flutter](https://img.shields.io/badge/Flutter-%2302569B.svg?style=flat&logo=Flutter&logoColor=white)
![React Native](https://img.shields.io/badge/react_native-%2320232a.svg?style=flat&logo=react&logoColor=%2361DAFB)
![Spring](https://img.shields.io/badge/spring-%236DB33F.svg?style=flat&logo=spring&logoColor=white)
![NodeJS](https://img.shields.io/badge/node.js-6DA55F?style=flat&logo=node.js&logoColor=white)
![Express.js](https://img.shields.io/badge/express.js-%23404d59.svg?style=flat&logo=express&logoColor=%2361DAFB)
![TailwindCSS](https://img.shields.io/badge/tailwindcss-%2338B2AC.svg?style=flat&logo=tailwind-css&logoColor=white)

**Backend & Tools:**
![GraphQL](https://img.shields.io/badge/-GraphQL-E10098?style=flat&logo=graphql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-%234ea94b.svg?style=flat&logo=mongodb&logoColor=white)
![Postgres](https://img.shields.io/badge/postgres-%23316192.svg?style=flat&logo=postgresql&logoColor=white)
![Firebase](https://img.shields.io/badge/firebase-a08021?style=flat&logo=firebase&logoColor=ffcd34)
![Git](https://img.shields.io/badge/git-%23F05033.svg?style=flat&logo=git&logoColor=white)
![Vercel](https://img.shields.io/badge/vercel-%23000000.svg?style=flat&logo=vercel&logoColor=white)
![Room DB](https://img.shields.io/badge/Room_DB-3DDC84?style=flat&logo=android&logoColor=white)
![Jetpack](https://img.shields.io/badge/Jetpack_Compose-4285F4?style=flat&logo=jetpackcompose&logoColor=white)

---

## 🎓 Education

* **B.Tech, Computer Science & Engineering** | Phonics Group Of Institutions, Roorkee (2023 – 2027)
* **Senior Secondary (XII)** | Bal Mandir Sec School, Haridwar (1st Division)
* **Secondary (X)** | Police Modern School Welfare Society, Haridwar (1st Division with Distinction)

---

*"Clean code is not written, it is evolved."*
